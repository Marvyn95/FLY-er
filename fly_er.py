...
def get_location_data(location = None):
    ...
    payload = {
        "input": location,
        'inputtype': "textquery",
        "key": "AIzaSyBFgMr9Nh5C-VNZE26PPEjyS-33VR89Qdk"
    }
    ...
    req2 = requests.get(url2, {"place_id": id, "key": "AIzaSyBFgMr9Nh5C-VNZE26PPEjyS-33VR89Qdk"})
...
def get_distance_data(origin_id=None, destination_id=None):
    ...
    try:
        ...
        distance_data = {
            ...
            "time_of_travel": reqdata["rows"][0]["elements"][0]["duration"]["text"]
            }
...
def closest_airfield(location, airport_list):
    """
    Args:
        location (dictionary): dictionary containing information on location
        airport_list (list of tuples): list of tuples having information a particular airstrip

    Returns:
        dictionary: returns a dictionary with information on the nearest airstrip found 
    """
    
    if airport_list is None:
        return {"Location_Error": "Try another location nearby"}
    
    print("getting closest airfield")
    #obtaining place_ids for all the airports
    ap_data = []
    ...
    #getting aiport closest to location
    try:
        closest_dist = float(get_distance_data(location["place_id"], ap_data[0]["place_id"])["distance_inbetween"].split(" ")[0])
        nearest_af = ap_data[0]
        index = 0
    except Exception as err:
        print("Error finding distance between airfields: ", err)
        return ({"Error": "No airfields found, please try a diffrent location nearby"})
    ...
    desired_as = {
        "Airfield_name": airport_list[index][0],
        "Town": airport_list[index][1],
        "District": nearest_af["District"],
        "Distance (km)": closest_dist,
        "ICAO": airport_list[index][-2],
        "IATA": airport_list[index][-1],
        "Coordinates": {"lat": airport_list[index][2], "long": airport_list[index][3]}
    }
